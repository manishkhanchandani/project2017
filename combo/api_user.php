...
$query_rsUserExist = sprintf("SELECT * FROM users_auth WHERE uid = '%s' and webReference='%s'", $colname_rsUserExist, WEBREFERENCE);
...
$totalRows_rsUserExist = mysql_num_rows($rsUserExist);
		...
		if ($totalRows_rsUserExist > 0) {
			...
			$updateSQL = sprintf("UPDATE users_auth SET display_name=%s, profile_img=%s, email=%s, logged_in_time=%s, profile_uid=%s WHERE user_id=%s",
		...
		if ($totalRows_rsUserExist == 0) {
			//INSERT THE USER IN THE DATABASE
			 $insertSQL = sprintf("INSERT INTO users_auth (display_name, profile_img, email, provider_id, user_created_dt, uid, logged_in_time, profile_uid, webReference) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($userData['displayName'], "text"),
                       GetSQLValueString($userData['photoURL'], "text"),
                       GetSQLValueString($userData['email'], "text"),
