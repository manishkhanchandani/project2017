...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form2")) {
  $insertSQL = sprintf("INSERT INTO users_auth (display_name, profile_img, email, provider_id, password, access_level, uid, logged_in_time, profile_uid, webReference) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
if (isset($_POST['MM_Login'])) {
  ...
  if ($loginFoundUser) {
    ...
    $_SESSION['MM_Username'] = $rec['email'];
...
<?php } ?>
		...
		<div class="form-group">
			<label for="display_name">Profile Image Url:</label>
			<input type="text" class="form-control" id="profile_img" name="profile_img" placeholder="Profile Image Url" value="<?php echo (!empty($_POST['profile_img'])) ? $_POST['profile_img'] : ''; ?>" />
		</div>
