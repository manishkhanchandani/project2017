...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE mk_users SET username=%s, name=%s, password=%s, accesslevel=%s, dob=%s, gender=%s, address1=%s, address2=%s, city=%s, province=%s, country=%s, zipcode=%s, homephone=%s, mobilephone=%s WHERE user_id=%s",
  ...
  $updateGoTo = "../timesheet/allusers.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_conn, $conn);
$query_rsEdit = sprintf("SELECT * FROM mk_users WHERE user_id = %s", $colname_rsEdit);
...
<form action="<?php echo $editFormAction; ?>" method="POST" name="form1" onSubmit="YY_checkform('form1','username','#q','0','Field \'email\' is not valid.','name','#q','0','Field \'name\' is not valid.','dob','^\([0-9]{4}\)\\-\([0-9][0-9]\)\\-\([0-9][0-9]\)$#3#2#1','3','Field \'dob\' is not valid.','password','#q','0','Field \'password\' is not valid.','cpassword','#password','6','Field \'confirm password\' is not valid.');return document.MM_returnValue">
  ...
  <p>Password:
      <input name="password" type="text" id="password" value="<?php echo $row_rsEdit['password']; ?>" size="12">
    Confirm Password:
    <input name="cpassword" type="text" id="cpassword" value="<?php echo $row_rsEdit['password']; ?>" size="12">
  </p>
  <p>Name:
  ...
  <p>Accesslevel: 
    <input name="accesslevel" type="text" id="accesslevel" value="<?php echo $row_rsEdit['accesslevel']; ?>">
</p>
  <p>
