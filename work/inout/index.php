...
$colname_rsOUT = "manish@mkgalaxy.com";
if (isset($_SESSION['MM_Username'])) {
  $colname_rsOUT = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_conn, $conn);
...
<form name="form2" method="POST" action="<?php echo $editFormAction; ?>" onSubmit="var ret=confirm('Do you really want to check out. Click ok to checkout'); if(ret) return true; else return false;">
  ...
  <p>Hours Worked Till Now: <?php
		...
		echo secondsToWords($diff);
		?>
		<br>
		<?php if((60*60*8)-$diff>0) {  //if((60*60*8)+(60*30)-$diff>0) {
		?>
		Pending Hours: 
		<?php
		$pending = (60*60*8)-$diff; //(60*60*8)+(60*30)-$diff;
		//$tmpTime = date("H:i",$pending); 
		//$arr = converttime($tmpTime);
		//echo $arr['time'];
		echo secondsToWords($pending);
   ?>
   <?php } else { ?>
