...
if (isset($_GET['username'])) {
  $colname_rsView = (get_magic_quotes_gpc()) ? $_GET['username'] : addslashes($_GET['username']);
}
$col1_rsView = "0";
if (isset($start)) {
...
$query_rsView = sprintf("SELECT * FROM mk_inout, mk_users WHERE mk_inout.user_id = mk_users.user_id AND mk_users.username LIKE '%s' AND (mk_inout.in_time BETWEEN %s AND %s) ORDER BY mk_inout.user_id ASC, mk_inout.inout_id DESC", $colname_rsView,$col1_rsView,$col2_rsView);
...
<form name="form1" method="get" action="">
  ...
  Today</label>
      <select name="username" id="username">
        <option value="%" <?php if (!(strcmp("%", $_GET['username']))) {echo "SELECTED";} ?>>Choose Username</option>
        <?php
do {  
?>
        <option value="<?php echo $row_rsUsers['username']?>"<?php if (!(strcmp($row_rsUsers['username'], $_GET['username']))) {echo "SELECTED";} ?>><?php echo $row_rsUsers['username']?></option>
        <?php
...
<table border="1" cellpadding="5" cellspacing="0">
  <tr>
    ...
    <td><strong>In IP </strong></td>
    <td><strong>Out IP </strong></td>
  </tr>
  <?php do { ?>
  <tr>
    <td><?php echo date('d M, Y',$row_rsView['in_time']); ?> </td>
    <td><?php echo $row_rsView['username']; ?></td>
...
 </td>
	...
	?> hours<?php } ?>
 </td>
    <td><a href="edit.php?inout_id=<?php echo $row_rsView['inout_id']; ?>&period=<?php echo $_GET['period']; ?>">Edit</a></td>
    <td><a href="delete.php?inout_id=<?php echo $row_rsView['inout_id']; ?>&period=<?php echo $_GET['period']; ?>" onClick="GP_popupConfirmMsg('Do you really want to delete this record?');return document.MM_returnValue">Delete</a></td>
    <td><?php echo $row_rsView['ip']; ?></td>
    <td><?php echo $row_rsView['ipout']; ?></td>
  </tr>
