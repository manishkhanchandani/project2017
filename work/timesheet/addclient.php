...
<?php
if($_POST['Submit']) {
	mysql_query("delete from mk_user_client where user_id = '".$_GET['user_id']."'") or die("error");
	if($_POST['list_id'][0]=="-1") {
	
	} else {
		foreach($_POST['list_id'] as $value) {
			mysql_query("insert into mk_user_client set user_id = '".$_GET['user_id']."', list_id = '".$value."'") or die("error");
		}
	}
}
$client = array();
...
$query_rstakeuser = sprintf("SELECT * FROM mk_users WHERE user_id = %s", $colname_rstakeuser);
...
mysql_select_db($database_conn, $conn);
$query_rsRel = sprintf("SELECT * FROM mk_user_client WHERE user_id = %s", $colname_rsRel);
...
mysql_select_db($database_conn, $conn);
$query_rsShow = sprintf("SELECT mk_list.list FROM mk_user_client, mk_list WHERE mk_list.list_id = mk_user_client.list_id AND mk_user_client.user_id = %s", $colname_rsShow);
...
<form name="form1" method="post" action="">
  <table border="1" cellspacing="0" cellpadding="5">
    <tr valign="top">
	  ...
	  <?php
	  $query = "SELECT * FROM mk_list WHERE pid = '0' ORDER BY list ASC";
